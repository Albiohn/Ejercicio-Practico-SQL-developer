-- Creacion de usuario 
CREATE USER ETMDY_FOL IDENTIFIED BY "UsuarioDEPRUEBA2025"
DEFAULT TABLESPACE "USERS"
TEMPORARY TABLESPACE "TEMP";
ALTER USER ETMDY_FOL QUOTA UNLIMITED ON USERS;
GRANT CREATE SESSION TO ETMDY_FOL;
GRANT "RESOURCE" TO ETMDY_FOL;
ALTER USER ETMDY_FOL DEFAULT ROLE "RESOURCE";
...
-- 1. Trigger para registrar cambios en la bitácora
CREATE OR REPLACE TRIGGER bitacora_sueldo_trigger
...
-- 2. Actualizaciones de sueldo base
...
-- 3. Definición del package
CREATE OR REPLACE PACKAGE ventas_package AS
  -- Variables públicas
  g_anio_proceso NUMBER := 2022; -- Año en proceso
  g_total_ventas NUMBER;
  g_error VARCHAR2(100);

  -- Función pública para retornar el monto de ventas del vendedor del año en proceso
  FUNCTION obtener_ventas_vendedor(p_rut_vendedor VARCHAR2) RETURN NUMBER;

  -- Procedimiento público para administrar errores
  PROCEDURE insertar_error(p_rutina_error VARCHAR2, p_descrip_error VARCHAR2);
END ventas_package;
/

-- 4. Cuerpo del package
CREATE OR REPLACE PACKAGE BODY ventas_package AS
  -- Función para obtener ventas de un vendedor
  FUNCTION obtener_ventas_vendedor(p_rut_vendedor VARCHAR2) RETURN NUMBER IS
    v_monto_ventas NUMBER;
  BEGIN
    SELECT NVL(SUM(total), 0) INTO v_monto_ventas
    FROM boleta
    WHERE rutvendedor = p_rut_vendedor AND EXTRACT(YEAR FROM fecha) = g_anio_proceso;
    RETURN v_monto_ventas;
  END obtener_ventas_vendedor;

  -- Procedimiento para insertar errores
  PROCEDURE insertar_error(p_rutina_error VARCHAR2, p_descrip_error VARCHAR2) IS
  BEGIN
    EXECUTE IMMEDIATE 'INSERT INTO error_procesos_mensuales (correl_error, rutina_error, descrip_error) VALUES (SEQ_ERROR.NEXTVAL, :1, :2)'
    USING p_rutina_error, p_descrip_error;
    COMMIT; -- Confirmar la transacción
  EXCEPTION
    WHEN OTHERS THEN
      RAISE; -- Re-lanzar la excepción para que sea capturada por bloques superiores
  END insertar_error;
END ventas_package;
/

-- Función almacenada fuera del package para obtener el monto total de ventas del año
CREATE OR REPLACE FUNCTION obtener_total_ventas_anio(p_anio NUMBER) RETURN NUMBER IS
  v_monto_total_ventas NUMBER;
BEGIN
  SELECT NVL(SUM(total), 0) INTO v_monto_total_ventas
  FROM boleta
  WHERE EXTRACT(YEAR FROM fecha) = p_anio;
  RETURN v_monto_total_ventas;
END obtener_total_ventas_anio;
/

-- Procedimiento almacenado fuera del package para generar el informe
CREATE OR REPLACE PROCEDURE generar_informe_porcentaje_vendedor IS
  v_monto_total_ventas NUMBER;
BEGIN
  -- Truncar la tabla de errores
  EXECUTE IMMEDIATE 'TRUNCATE TABLE error_procesos_mensuales';
  -- Obtener el monto total de ventas del año en proceso
  v_monto_total_ventas := obtener_total_ventas_anio(ventas_package.g_anio_proceso);

  IF v_monto_total_ventas > 0 THEN
    FOR v_record IN (SELECT rutvendedor, nombre, (SELECT descripcion FROM comuna WHERE codcomuna = v.codcomuna) AS comuna, sueldo_base
                     FROM vendedor v) LOOP
      BEGIN
        INSERT INTO porcentaje_vendedor (anio, rutvendedor, nomvendedor, comuna, sueldo_base, aporte_ventas)
        VALUES (ventas_package.g_anio_proceso, v_record.rutvendedor, v_record.nombre, v_record.comuna, v_record.sueldo_base,
                (ventas_package.obtener_ventas_vendedor(v_record.rutvendedor) / v_monto_total_ventas));
      EXCEPTION
        WHEN OTHERS THEN
          ventas_package.insertar_error('generar_informe_porcentaje_vendedor', SQLERRM);
      END;
    END LOOP;
  ELSE
    ventas_package.insertar_error('generar_informe_porcentaje_vendedor', 'El monto total de ventas del año es cero.');
  END IF;
END generar_informe_porcentaje_vendedor;
/

-- ejecucion, es necesario repetirla una vez luego de la primera para obtener los errores solicitados en la tabla, dicho de otra forma, es necesario forzar el error--

-- 5. Bloque anónimo para ejecutar el procedimiento almacenado que genera el informe
BEGIN
